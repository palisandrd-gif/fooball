...
API_FOOTBALL_MIN_INTERVAL_MS=6500
```
...
Бот выдерживает минимальный интервал 6,5 секунды между запросами, чтобы соответствовать бесплатному лимиту 10 запросов в минуту. Если пользователь запрашивает матч, которого ещё нет в локальном кэше, бот находит его по дате, один раз загружает доступную статистику и сохраняет результат в PostgreSQL. Повторные просмотры внешнюю квоту не расходуют.
...
6. Найдите команду через `/team` и нажмите «📊 Подробности матча». Первый просмотр может занять 10–20 секунд, последующие берутся из кэша.
